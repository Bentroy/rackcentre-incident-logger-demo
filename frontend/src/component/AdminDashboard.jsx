    ...
    <div className="flex h-screen bg-gray-100 text-gray-800">
      ...
      <aside className="w-64 bg-white shadow-md flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2">
           <a href="#" className="w-full flex items-center px-4 py-2 rounded-lg bg-blue-100 text-blue-700 font-medium">
             📊 Dashboard
           </a>
        </nav>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="p-6 space-y-8">
          {/* Stats Section */}
          <section>
            <h2 className="text-xl font-semibold mb-4 text-slate-700">Analytics Overview</h2>
            {loading ? (
                <p>Loading stats...</p>
            ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="flex justify-around items-center mt-2">
                        {Object.entries(stats.byImpact).map(([impact, count]) => (
                            <div key={impact} className="text-center">
                                <p className={`text-3xl font-bold ${impactColors[impact].replace('bg-', 'text-').split(' ')[0]}`}>{count}</p>
                ...
                {/* Stats by Type */}
                <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200/80">
                    <h3 className="text-gray-500 font-medium">Most Frequent Type</h3>
                    <div className="flex items-baseline space-x-2 mt-2">
                        <p className="text-3xl font-bold text-indigo-600">
                         {Object.keys(stats.byType).length > 0 ? Object.entries(stats.byType).reduce((a, b) => b[1] > a[1] ? b : a)[0] : 'N/A'}
                        </p>
                    </div>
                </div>
            </div>
          ...
          <section>
             <h2 className="text-xl font-semibold mb-4 text-slate-700">All Incident Reports</h2>
            {/* Filter and Sort Controls */}
             <div className="bg-white p-4 rounded-xl shadow-lg border border-gray-200/80 mb-6 flex flex-wrap gap-4 items-center">
                ...
                <select name="impact" onChange={handleFilterChange} className="p-2 border rounded-md bg-gray-50">
                ...
                <select name="type" onChange={handleFilterChange} className="p-2 border rounded-md bg-gray-50">
                ...
                <select name="userId" onChange={handleFilterChange} className="p-2 border rounded-md bg-gray-50">
                ...
                <div className="flex-grow"></div>
                 <button onClick={() => handleSortChange('date')} className="p-2 text-sm text-gray-600 font-semibold">
                    Sort by Date {sortConfig.key === 'date' && (sortConfig.direction === 'descending' ? '▼' : '▲')}
                </button>
                <button onClick={() => handleSortChange('impact')} className="p-2 text-sm text-gray-600 font-semibold">
                    Sort by Impact {sortConfig.key === 'impact' && (sortConfig.direction === 'descending' ? '▼' : '▲')}
                </button>
             </div>

            {loading ? (
              <p className="text-center text-gray-500 py-10">Loading incidents...</p>
            ) : filteredAndSortedIncidents.length === 0 ? (
              <p className="text-center text-gray-500 py-10">No incidents match the current filters.</p>
            ) : (
              <div className="space-y-4">
                {filteredAndSortedIncidents.map((incident) => (
                  <div key={incident._id} className="bg-white p-4 rounded-xl shadow-md border border-gray-200/80 transition hover:shadow-lg">
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
                      <div className="md:col-span-2">
                        <p className="font-bold text-lg text-slate-800">{incident.title}</p>
                        {/* THIS IS THE NEW PART: Displaying the user's name */}
                        <p className="text-sm text-gray-500">Reported by: <span className="font-semibold">{incident.user.name}</span></p>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-xl">{typeIcons[incident.type]}</span>
                        <span className="px-2 py-1 text-xs font-semibold bg-indigo-100 text-indigo-700 rounded-full">{incident.type}</span>
                      </div>
                      <div>
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full border ${impactColors[incident.impact]}`}>
